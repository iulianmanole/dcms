...
class Inno_Model_ResultSet implements Countable, Iterator
{
	/**
	...
	public function __construct($results)
	{
		if (!$this->_resultClass) {
			throw new Inno_Model_Exception('No result class specified!');
		}

		if (!is_array($results) && (!$results instanceof Traversable)) {
			throw new Inno_Model_Exception('Invalid result set; must be array or Traversable');
		}
